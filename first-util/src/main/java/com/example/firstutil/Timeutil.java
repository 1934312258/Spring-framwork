...
//        }
        ...
        //ava 8增加了一个Clock时钟类用于获取当时的时间戳，或当前时区下的日期时间信息。
        // 以前用到System.currentTimeInMillis()和TimeZone.getDefault()的地方都可用Clock替换。
        Clock clock = Clock.systemDefaultZone();
        //clock.instant()获取当前时间，clock.millis()获取时间毫秒
        ...
        //转换时间为另一时区
        LocalDateTime dateTime = LocalDateTime.now(ZoneId.systemDefault());//直接加入时区即可获得相应时区的时间
        ZonedDateTime american = ZonedDateTime.now(ZoneId.of("America/New_York"));//获取时区内当前时间、偏移量与时区的ZonedDateTime实例
        ZonedDateTime zonedate = ZonedDateTime.of(LocalDateTime.now(), zone);//获取本地时间、时区的ZonedDateTime实例，不明白什么意思
